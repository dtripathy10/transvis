...
import edu.iitb.tse.algo.core.tripdistribution.growthfactor.SinglyConstrinedGrowthFactor;
...
@ActionID(
    category = "Edit",
id = "edu.iit.tse.transvis.experimentui.tripdistribution.SingleConstrainedGrowthFactorModel")
@ActionRegistration(
    displayName = "#CTL_SingleConstrainedGrowthFactorModel")
@ActionReference(path = "Menu/Experiment/TripDistribution", position = 3333)
@Messages("CTL_SingleConstrainedGrowthFactorModel=Single Constrained GrowthFactor Model")
public final class SingleConstrainedGrowthFactorModel implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        io.getErr().println("Single Constrained GrowthFactor Model is startd");
        io.getErr().println("----------------------------------------------");
        io.select();

        SingleConstrainedGrowthFactorDialog d = new SingleConstrainedGrowthFactorDialog(WindowManager.getDefault().getMainWindow(), true);
        d.setTitle("Single Constrained GrowthFactor | Trip Distribution");
        d.setLocationRelativeTo(null);
        d.setVisible(true);

        //get the data
        final FileObject baseFile = d.getBaseyearMatrix();
        final FileObject horizonArray = d.getHorizonArray();
        ConstrainedType convergence = d.getConvergence();
        System.out.println(baseFile);
        System.out.println(horizonArray);
        System.out.println(convergence);
        int[][] baseYear = null;
        int[] horizonYear = null;
        //read the file
        try {
        ...
        try {
            bufRdr = new BufferedReader(new FileReader(FileUtil.toFile(horizonArray)));
            String[] row = null;
            ...
            while ((line = bufRdr.readLine()) != null) {
                ...
                if (line.trim().startsWith("#")) {
                    ...
                    horizonYear = new int[str.length];
                    for (int i = 0; i < str.length; i++) {
                        horizonYear[i] = Integer.parseInt(str[i]);
                    }
                }
            }
            bufRdr.close();
        ...
        //perform experiment
        SinglyConstrinedGrowthFactor uniformGrowthFactor = new SinglyConstrinedGrowthFactor(baseYear, horizonYear, convergence);
