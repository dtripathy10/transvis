...
import edu.iitb.tse.algo.core.SystemOptimalAssignment;
...
@ActionID(
    category = "Edit",
id = "edu.iit.tse.transvis.experimentui.SystemOptimal")
@ActionRegistration(
    //iconBase = "edu/iit/tse/transvis/experimentui/systemoptimal.png",
displayName = "#CTL_SystemOptimal")
@ActionReference(path = "Menu/Experiment/TripAssignment", position = 3183)
@Messages("CTL_SystemOptimal=System Optimal")
public final class SystemOptimal implements ActionListener {
...
//         required the algorithm*/
        ...
        io.getErr().println("System Optimal experiment is startd");
        io.getErr().println("----------------------------------------------");
        io.select();
//
//        //create the network
        ...
        d.setTitle("System Optimal | Trip Assignment");
        ...
        SystemOptimalAssignment aon = new SystemOptimalAssignment(network, odTable);
        ...
        try {
            ...
            obj = FileUtil.createData(file.getFileObject("input"), networkFile.getName() + "_sop.csv");
        } catch (IOException ex) {
            Exceptions.printStackTrace(ex);
        }
        PrintWriter pw = null;
        ...
        WindowManager.getDefault().invokeWhenUIReady(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    //                vtp.open();
                    //                vtp.requestActive();
                    // DataObject dob = DataObject.find(ProjectInformation.getCurrentProject().getProjectDirectory().getFileObject("input").getFileObject("result.csv"));
                    DataObject dob = DataObject.find(ProjectInformation.getCurrentProject().getProjectDirectory().getFileObject("input").getFileObject(networkFile.getName() + "_sop.csv"));
