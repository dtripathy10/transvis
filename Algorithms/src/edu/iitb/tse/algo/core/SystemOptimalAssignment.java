 ...
 * @author dtripathy10
 */
public class SystemOptimalAssignment {
    ...
    public SystemOptimalAssignment(Network network, ODTable odtable) {
    ...
    public double derivative(Map<Link, Double> xVector, Map<Link, Double> yVector, double alpha, Map<Link, Double> baseTime) {
        ...
        double deriv = 0.0;
        for (Link link : network.getLinks()) {
            double time,normalTime,derivateOftime;
            double newFlow = xVector.get(link) + alpha * (yVector.get(link) - xVector.get(link));
            if (newFlow < TOLERANCE) {
                time = baseTime.get(link);
            } else {
                normalTime = time(link, xVector, baseTime);
                derivateOftime = derivativeOftime(link, newFlow, baseTime);
                time = newFlow * derivateOftime + normalTime;
            }
    ...
    private double derivativeOftime(Link link, double flow, Map<Link, Double> baseTime) {
        final double TOLERANCE = EPSLON;		// Square root of machine precision
        double time;
        double xtime = baseTime.get(link);
        if (flow < TOLERANCE) {
            ...
            double delay = ((ALPHA * BETA) / Math.pow(link.getCapacity(), BETA - 1)) * Math.pow(flow, BETA - 1);
            time = xtime * delay;
        }
        return time;
    }
    ...
    public void setMAX_ITERATIONS(int numberOfIteration) {
